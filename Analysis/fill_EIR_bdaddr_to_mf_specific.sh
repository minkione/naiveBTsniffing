...
echo "mysql import"
mysql -u root --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_mf_specific_uniq.csv' REPLACE INTO TABLE EIR_bdaddr_to_mf_specific FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (device_bdaddr, @device_BT_CID, mf_specific_data) SET device_BT_CID = CAST(CONV(REPLACE(@device_BT_CID, '0x', ''), 16, 10) AS UNSIGNED);"
