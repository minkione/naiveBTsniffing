...
echo "mysql import"
mysql -u root --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_flags_uniq.csv' REPLACE INTO TABLE EIR_bdaddr_to_flags FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (device_bdaddr, le_limited_discoverable_mode, le_general_discoverable_mode, le_bredr_support_controller, le_bredr_support_host);"
