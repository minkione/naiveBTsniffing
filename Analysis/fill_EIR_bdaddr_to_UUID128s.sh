...
echo "mysql import"
mysql -u root --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_UUID128s_incomplete_uniq.csv' REPLACE INTO TABLE EIR_bdaddr_to_UUID128s FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (device_bdaddr, str_UUID128s) SET list_type = 6;"
mysql -u root --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_UUID128s_complete_uniq.csv' REPLACE INTO TABLE EIR_bdaddr_to_UUID128s FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (device_bdaddr, str_UUID128s) SET list_type = 7;"
