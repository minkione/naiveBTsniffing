...
echo "mysql import"
mysql -u root --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_name_short_uniq.csv'  IGNORE INTO TABLE EIR_bdaddr_to_name FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (device_bdaddr, device_name) SET device_name_type = 8;"
mysql -u root --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_name_complete_uniq.csv'  IGNORE INTO TABLE EIR_bdaddr_to_name FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (device_bdaddr, device_name) SET device_name_type = 9;"
