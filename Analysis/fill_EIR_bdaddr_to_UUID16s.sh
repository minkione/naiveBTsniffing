...
echo "mysql import"
mysql -u root --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_UUID16s_incomplete_uniq.csv' REPLACE INTO TABLE EIR_bdaddr_to_UUID16s FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (device_bdaddr, str_UUID16s) SET list_type = 2;"
mysql -u root --database='bt' --execute="LOAD DATA INFILE '/tmp/EIR_bdaddr_to_UUID16s_complete_uniq.csv' REPLACE INTO TABLE EIR_bdaddr_to_UUID16s FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' (device_bdaddr, str_UUID16s) SET list_type = 3;"
